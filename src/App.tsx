...
export default function App() {
  ...
  const handleGenerateExam = async () => {
    ...
    try {
      ...
      if (vocabSource === "system") {
        ...
        finalVocabList = selectedSystemSubset.map(w => ({ word: w.word, pos: w.pos, meaning: w.meaning }));
  ...
  const handleInteractiveSubmitAnswers = async () => {
    ...
    // 2. Cloze
    if (examSuite.clozeSuite && examSuite.clozeSuite.questions) {
    ...
    if (examSuite.blankMatchingSuite) {
      examSuite.blankMatchingSuite.answers.forEach((ans, idx) => {
        ...
        const isCorrect = userAns === normalizeAnswer(ans);
        ...
        reportDetails.push({
          ...
          isCorrect,
          userAnswer: userAns,
          correctAnswer: normalizeAnswer(ans),
          questionText: `文意選填第 (${idx + 21}) 答案格`
        });
      });
          ...
          <nav className="flex items-center gap-1 sm:gap-2 text-xs">
            {examSuite && (
              <>
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-semibold transition duration-150 flex items-center gap-1.5 ${activeTab === "player" ? "bg-teal-50 text-teal-850" : "text-stone-600 hover:bg-stone-50"}`}
                  id="nav-player-btn"
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg font-semibold transition duration-150 flex items-center gap-1.5 ${activeTab === "worksheet" ? "bg-amber-50 text-amber-900" : "text-stone-600 hover:bg-stone-50"}`}
            ...
            {activeReport && (
              <button
                ...
                className={`px-3 py-2 rounded-lg font-semibold transition duration-150 flex items-center gap-1.5 ${activeTab === "report" ? "bg-stone-850 text-white" : "text-stone-600 hover:bg-stone-50"}`}
                id="nav-report-btn"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-8 bg-white border border-stone-200/90 rounded-2xl p-6 md:p-8 shadow-xs space-y-6">
                <div className="border-b border-stone-150 pb-4">
                  ...
                  <p className="text-xs text-stone-500 mt-1">選定本測驗的核心字表字級、篩選單元，或直接自主貼上自訂單字表。</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 p-1.5 bg-stone-100 rounded-xl" id="vocab-source-container">
                  <button
                    ...
                    className={`py-2 rounded-lg text-xs font-bold transition duration-200 flex items-center justify-center gap-1.5 ${vocabSource === "system" ? "bg-white text-stone-900 shadow-xs" : "text-stone-500 hover:text-stone-800"}`}
                  ...
                  <button
                    ...
                    className={`py-2 rounded-lg text-xs font-bold transition duration-200 flex items-center justify-center gap-1.5 ${vocabSource === "self-input" ? "bg-white text-stone-900 shadow-xs" : "text-stone-500 hover:text-stone-800"}`}
                    id="source-self-btn"
                  >
                ...
                {vocabSource === "system" ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-stone-50/50 p-4 rounded-xl border border-stone-100">
                    <div className="space-y-1.5" id="level-select-container">
                      <label className="text-xs font-bold font-sans uppercase text-stone-500">Syllabus Level (篩選學測字級)</label>
                      <select
                        value={selectedLevel}
                    ...
                    <div className="space-y-1.5 sm:col-span-2 border-t border-stone-100 pt-3 mt-1">
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-bold font-sans uppercase text-stone-500">No. of Words to Extract (單字提取數量)</label>
                        <span className="font-mono text-xs font-bold text-amber-800 bg-amber-50 px-2 py-0.5 rounded">{randomWordCount} words</span>
                      </div>
                      <input
                        type="range" min="5" max="50" value={randomWordCount}
                  ...
                  <div className="space-y-2 bg-stone-50/50 p-4 rounded-xl border border-stone-100" id="self-input-container">
                    <div className="flex justify-between items-start gap-4 flex-wrap">
                      ...
                      <span className="text-[10px] text-amber-900 font-bold bg-amber-50 px-2 py-1 rounded">Format: word POS (one per line) / 格式：單字 詞性</span>
                    </div>
                    <textarea
                      value={selfInputText}
                      onChange={(e) => { setSelfInputText(e.target.value); setSelfInputError(null); }}
                      rows={6}
                    ...
                    <div className="flex justify-between items-center text-[10px] text-stone-500 pt-1 flex-wrap gap-2">
                      ...
                      <span className="text-stone-400">(若少於 12 字，系統將自動隨機補足同級單字。)</span>
                    </div>
                  </div>
                )}
                ...
                <div className="border-t border-stone-150 pt-6 space-y-4">
                  <div className="pb-1">
                    ...
                    <p className="text-xs text-stone-500 mt-1">勾選欲產生的學測模擬大題組合。</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4" id="quiz-types-checklist">
                    ...
                    <label className={`border rounded-xl p-4 flex items-center gap-3 cursor-pointer transition ${selectedExerciseTypes.cloze ? "border-teal-400 bg-teal-50/20" : "border-stone-200 hover:border-stone-300 bg-white"}`}>
                      <input type="checkbox" checked={selectedExerciseTypes.cloze} onChange={() => setSelectedExerciseTypes(prev => ({ ...prev, cloze: !prev.cloze }))} className="rounded border-stone-300 text-teal-700 focus:ring-teal-700 w-4 h-4 shrink-0" id="checkbox-cloze-test" />
                      <span className="text-xs font-bold text-stone-900 font-sans">GSAT Cloze / Integrated Gaps (5 Qs) / 綜合測驗 (5 格)</span>
                    </label>

                    <label className={`border rounded-xl p-4 flex items-center gap-3 cursor-pointer transition ${selectedExerciseTypes.blankMatching ? "border-teal-400 bg-teal-50/20" : "border-stone-200 hover:border-stone-300 bg-white"}`}>
                      <input type="checkbox" checked={selectedExerciseTypes.blankMatching} onChange={() => setSelectedExerciseTypes(prev => ({ ...prev, blankMatching: !prev.blankMatching }))} className="rounded border-stone-300 text-teal-700 focus:ring-teal-700 w-4 h-4 shrink-0" id="checkbox-blank-matching" />
                      <span className="text-xs font-bold text-stone-900 font-sans">GSAT Blank Matching (10 Gaps) / 文意選填 (10 格)</span>
                    </label>
                    ...
                    <div className={`border rounded-xl p-4 space-y-3 transition ${selectedExerciseTypes.reading ? "border-teal-400 bg-teal-50/20" : "border-stone-200 hover:border-stone-300 bg-white"}`}>
                      <label className="flex items-center gap-3 cursor-pointer">
                        ...
                        <span className="text-xs font-bold text-stone-900 font-sans">GSAT Reading Comprehension (4 Qs) / 閱讀測驗 (4 題)</span>
                      </label>
                          ...
                          {["basic", "essential", "advanced"].map(lvl => {
                            ...
                            return (
                              <button
                                key={lvl} type="button"
                                onClick={() => {
                                  if (isLvlChecked) setSelectedReadingLevels(selectedReadingLevels.filter(l => l !== lvl));
                                  else setSelectedReadingLevels([...selectedReadingLevels, lvl]);
                                }}
                                className={`py-1.5 px-2 rounded-lg text-[10px] border font-bold capitalize transition duration-150 ${isLvlChecked ? "bg-teal-800 text-white border-teal-800" : "bg-white text-stone-600 border-stone-200 hover:bg-stone-50"}`}
                ...
                <div className="border-t border-stone-150 pt-6">
                  ...
                  {generationLoading ? (
                    ...
                    <button
                      ...
                      className="w-full bg-teal-800 hover:bg-teal-900 text-white rounded-2xl py-4 font-semibold text-sm flex items-center justify-center gap-2 shadow-md active:scale-[0.99] transition duration-200"
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto" id="history-logs-shelf">
                      {studyHistory.map((log) => (
                        <div
                          ...
                          onClick={() => { setActiveReport(log); setActiveTab("report"); }}
                          id={`history-log-${log.sessionId}`}
                        >
                          <div className="space-y-0.5">
                            <span className="text-[10px] text-stone-500 font-mono">{new Date(log.timestamp).toLocaleDateString()} Completed</span>
                            <p className="text-xs font-bold font-display text-stone-900">Accuracy: {log.scoreSummary.comprehensive.score}%</p>
                            <span className="text-[10px] text-stone-500 font-sans block">Correct: {log.scoreSummary.comprehensive.correct}/{log.scoreSummary.comprehensive.total}</span>
                          </div>
          ...
          <div className="space-y-6 animate-fade-in" id="quiz-player-dashboard">
            ...
            <div className="bg-white border border-stone-200 rounded-2xl p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <button onClick={() => setActiveTab("worksheet")} className="px-4 py-2 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 border border-stone-200/80 transition" id="player-to-worksheet-tab-btn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-3 flex flex-col gap-2 bg-white p-3 border border-stone-200 rounded-2xl shadow-xs" id="player-sections-nav">
                ...
                {examSuite.clozeSuite && (
                  <button onClick={() => setCurrentSection("cloze")} className={`px-3 py-2.5 rounded-xl text-xs font-bold flex items-center justify-between transition ${currentSection === "cloze" ? "bg-teal-700 text-white" : "text-stone-600 hover:bg-stone-50"}`} id="section-nav-cloze">
                ...
                {examSuite.blankMatchingSuite && (
                  <button onClick={() => setCurrentSection("matching")} className={`px-3 py-2.5 rounded-xl text-xs font-bold flex items-center justify-between transition ${currentSection === "matching" ? "bg-teal-700 text-white" : "text-stone-600 hover:bg-stone-50"}`} id="section-nav-matching">
                    <span>Part III: 文意選填</span>
                ...
                <div className="border-t border-stone-100 pt-3 mt-4">
                  <button onClick={handleInteractiveSubmitAnswers} className="w-full bg-teal-800 hover:bg-teal-900 text-white text-xs font-semibold py-2 rounded-xl transition shadow-xs" id="submit-side-btn">
                    Diagnose Grade Now
                  </button>
                </div>
              </div>
              ...
              <div className="lg:col-span-9 bg-white border border-stone-200 p-6 md:p-8 rounded-2xl shadow-xs" id="player-exercise-viewport">
                ...
                {currentSection === "vocab" && examSuite.vocabQuestions && (
                  <div className="space-y-6" id="player-vocab-section">
                    <div className="border-b border-stone-100 pb-3 flex justify-between items-center">
                      <h3 className="text-base font-bold font-display text-stone-900">Part I: Vocabulary MCQ (學測字彙單選題)</h3>
                ...
                {currentSection === "cloze" && examSuite.clozeSuite && examSuite.clozeSuite.questions && (
                  <div className="space-y-6" id="player-cloze-section">
                    <div className="border-b border-stone-100 pb-3">
                      <h3 className="text-base font-bold font-display text-stone-900">Part II: Cloze Test (學測綜合測驗)</h3>
                ...
                {currentSection === "matching" && examSuite.blankMatchingSuite && (
                  <div className="space-y-6" id="player-matching-section">
                    <div className="border-b border-stone-100 pb-3">
                      <h3 className="text-base font-bold font-display text-stone-900">Part III: Blank Matching (學測文意選填)</h3>
                      <p className="text-xs text-stone-500 mt-0.5">為文章中標號空格 21–30 選出最適合的候選詞。每個選項僅限使用一次。</p>
                    </div>
                    ...
                    <div className="bg-stone-100 border border-stone-200 rounded-xl p-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 font-mono text-xs font-semibold">
                        {normalizeOptions(examSuite.blankMatchingSuite.options).map((opt, idx) => {
                          ...
                          return (
                            <div key={idx} className={`py-2 px-3 rounded-lg border text-center transition ${isUsed ? "bg-stone-200/50 text-stone-400 line-through border-stone-250 select-none" : "bg-white text-stone-800 border-stone-300 shadow-xs"}`}>
                              {opt}
                            </div>
                          );
                        })}
                    ...
                    {/* Passage */}
                    <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 md:p-6 text-base font-sans leading-loose text-stone-800 whitespace-pre-wrap">
                      {examSuite.blankMatchingSuite.passage}
                    </div>

                    {/* Selectors for gaps 21–30 */}
                      ...
                      {Array.from({ length: 10 }).map((_, idx) => {
                        ...
                        return (
                          <div key={idx} className="bg-white border border-stone-200 rounded-xl p-4 flex justify-between items-center gap-3">
                            <div>
                              <span className="text-xs font-bold text-stone-900 font-display">Blank __ {gapNo} __</span>
                              <p className="text-[10px] text-stone-400 font-sans mt-0.5">Choose the best candidate for blank {gapNo}</p>
                            </div>
                            <select
                              value={userSel}
                              onChange={(e) => {
                                const val = e.target.value;
                                setSession(prev => ({ ...prev, answers: { ...prev.answers, blankMatching: { ...prev.answers.blankMatching, [idx]: val } } }));
                              }}
                            ...
                            >
                              ...
                              {["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"].map((letter) => {
                                ...
                                return <option key={letter} value={letter}>{fullOptStr}</option>;
                              })}
                            </select>
                          </div>
                        );
                ...
                {currentSection === "reading" && examSuite.readingPassages && (
                  <div className="space-y-8" id="player-reading-section">
                    <div className="border-b border-stone-100 pb-3">
                      <h3 className="text-base font-bold font-display text-stone-900">Part IV: Reading Comprehension (學測閱讀測驗)</h3>
                      <p className="text-xs text-stone-500 mt-0.5">仔細閱讀文章後作答 4 題閱讀理解單選題。</p>
                    </div>
                      ...
                      <div key={pIdx} className="space-y-6 border-b border-stone-200 pb-8 last:border-none">
                        ...
                        <div className="space-y-6 pt-4">
                          {p.questions.map((q, qIdx) => {
                            ...
                            const userAns = session.answers.reading[userKey] || "";
                            return (
                              <div key={q.id} className="bg-stone-50/50 p-4 rounded-xl border border-stone-150/50 space-y-3">
